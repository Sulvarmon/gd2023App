...
export default function AboutCookies({font0, font1, cookies, aboutCookies, setAboutCookies}) {
    ...
    return (
        <div onClick={() => { setAboutCookies(false) }} className={`${styles.aboutCookies} ${!aboutCookies && 'dn' }`}>
            ...
            <div onClick={(e)=>{e.stopPropagation()}} className={`${styles.cookiesTexts} dfcjcac gap4 container mt5 p2 br2 cw`}>
                ...
                <div className={`${font1}`}>{cookies['cookie texts']['texts'][5]} <a href='https://allaboutcookies.org' target='__blank' className={`${font1} colorBlue tdu di`}><b>{cookies['cookie texts']['texts'][6]}</b></a></div>
            </div>
        </div>
    )
}
