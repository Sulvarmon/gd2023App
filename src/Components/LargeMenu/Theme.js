...
export default function Theme({ languageData }) {
  ...
  return (
    <div className={`dfcjcac gap1 w5`}>
      <div onClick={() => { setShowTheme(!showTheme) }} className={`${styles.lang} usn dfjcac gap1 cp p2`}><span className={`${languageData['font-family'][0]}`}>{languageData['theme'][2]}</span><SlArrowDown className={`${showTheme ? styles.tr0 : styles.tr90}`} /></div>
      <div className={`${showTheme ? '' : 'dn'} w5`}>
        <hr className='w5' />
        <div onClick={() => { }} className={` dfcjcac gap2 mt2 mb2`}>
          <span className={`${languageData['font-family'][0]}`}>{languageData['theme'][1]}</span>
          ...
          <span className={`${languageData['font-family'][0]}`}>{languageData['theme'][2]}</span>
        </div>
        <hr className='w5' />
      </div>
    </div>
